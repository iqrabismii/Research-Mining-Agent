...
var chunkMEMYFFOL_cjs = require('./chunk-MEMYFFOL.cjs');
...
var chunkPP4G2TZZ_cjs = require('./chunk-PP4G2TZZ.cjs');
var chunkOOTLMVNN_cjs = require('./chunk-OOTLMVNN.cjs');
...
var ProcessorRunner = class _ProcessorRunner {
  ...
  async runOutputProcessors(messageList, observabilityContext, requestContext, retryCount = 0, writer) {
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      if (result instanceof chunkYZMSJKAK_cjs.MessageList) {
  ...
  static async validateAndFormatProcessInputStepResult(result, {
    ...
    if (result instanceof chunkYZMSJKAK_cjs.MessageList) {
      ...
      if (result.model) {
        const resolvedModel = await chunkPP4G2TZZ_cjs.resolveModelConfig(result.model);
            ...
            case "finish":
              ...
              try {
                if (self.processorRunner && !self.#options.isLLMExecutionStep) {
                  ...
                  const outputText = lastResponseMessage ? chunkYZMSJKAK_cjs.coreContentToString(lastResponseMessage.content) : "";
  ...
  switch (chunk.type) {
    ...
    case "file":
      if (mode === "generate") {
        return {
          type: "file",
          file: new chunkYZMSJKAK_cjs.DefaultGeneratedFile({
      ...
      return {
        type: "file",
        file: new chunkYZMSJKAK_cjs.DefaultGeneratedFileWithType({
          data: chunk.payload.data,
          mediaType: chunk.payload.mimeType
        })
      };
...
async function validateStepInput({
  ...
  let validationError;
  if (validateInputs && chunkGCRPNAAR_cjs.isZodType(step.inputSchema)) {
...
async function validateStepResumeData({ resumeData, step }) {
  ...
  if (resumeSchema && chunkGCRPNAAR_cjs.isZodType(resumeSchema)) {
...
async function validateStepSuspendData({
  ...
  if (suspendSchema && validateInputs && chunkGCRPNAAR_cjs.isZodType(suspendSchema)) {
...
async function validateStepStateData({
  ...
  if (stateSchema && validateInputs && chunkGCRPNAAR_cjs.isZodType(stateSchema)) {
...
async function validateStepRequestContext({
  ...
  if (requestContextSchema && validateInputs && chunkGCRPNAAR_cjs.isZodType(requestContextSchema)) {
...
var createTimeTravelExecutionParams = (params) => {
  ...
  for (const [index, entry] of graph.steps.entries()) {
    ...
    stepIds.forEach((stepId) => {
      ...
      if (result) {
        const formattedResult = chunkGCRPNAAR_cjs.removeUndefinedValues(result);
  ...
  do {
    ...
    const evalSpan = await engine.createChildSpan({
      ...
      options: {
        ...
        input: chunkGCRPNAAR_cjs.selectFields(result.output, ["stepResult", "output.text", "output.object", "messages"]),
        attributes: {
...
function isToolStep(input) {
  return input instanceof chunkM26GEN4C_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof Agent) && !(input instanceof chunkM26GEN4C_cjs.Tool);
}
  ...
  return {
    ...
    execute: async ({ inputData, requestContext, tracingContext, outputWriter }) => {
      ...
      const passThrough = {
        ...
        messageList: messageList ?? (Array.isArray(messages) ? new chunkYZMSJKAK_cjs.MessageList().add(messages, "input").addSystem(systemMessages ?? []) : void 0),
        stepNumber,
        systemMessages,
        streamParts,
        state,
...
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof Agent) && !(obj instanceof chunkM26GEN4C_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
var Run = class {
  ...
  async _validateInput(inputData) {
    ...
    if (this.validateInputs && this.inputSchema && chunkGCRPNAAR_cjs.isZodType(this.inputSchema)) {
  ...
  async _validateInitialState(initialState) {
    ...
    if (this.validateInputs) {
      ...
      if (stateSchema && chunkGCRPNAAR_cjs.isZodType(stateSchema)) {
  ...
  async _validateRequestContext(requestContext) {
    if (this.validateInputs && this.requestContextSchema && chunkGCRPNAAR_cjs.isZodType(this.requestContextSchema)) {
      const contextValues = requestContext?.all ?? {};
...
` + errors.map((e) => `- ${e.path?.join(".")}: ${e.message}`).join("\n")
  ...
  async _validateResumeData(resumeData, suspendedStep) {
    ...
    if (suspendedStep && suspendedStep.resumeSchema && this.validateInputs && chunkGCRPNAAR_cjs.isZodType(suspendedStep.resumeSchema)) {
  ...
  async _validateTimetravelInputData(inputData, step) {
    let inputDataToUse = inputData;
    if (step && step.inputSchema && this.validateInputs && chunkGCRPNAAR_cjs.isZodType(step.inputSchema)) {
      const inputSchema = step.inputSchema;
...
var MastraScorer = class _MastraScorer {
  ...
  async executePromptStep(scorerStep, observabilityContext, context) {
    ...
    const resolvedModel = await chunkPP4G2TZZ_cjs.resolveModelConfig(modelConfig, void 0, this.#mastra);
...
Analyse the suspended tools: ${JSON.stringify(suspendedTools)}, using the messages available to you and the resumeSchema of each suspended tool, find the tool whose resumeData you can construct properly.
        ...
        if (currentStep.model instanceof chunkPP4G2TZZ_cjs.ModelRouterLanguageModel) {
              ...
              parts: errorResults.map((toolCallErrorResult) => {
                return {
                  ...
                  toolInvocation: {
                    ...
                    toolName: chunkYZMSJKAK_cjs.sanitizeToolName(toolCallErrorResult.toolName),
                    ...
                    clientResults.map(async (toolCall) => {
                      ...
                      return {
                        ...
                        toolInvocation: {
                          state: "result",
                          toolCallId: toolCall.toolCallId,
                          toolName: chunkYZMSJKAK_cjs.sanitizeToolName(toolCall.toolName),
                          args: toolCall.args,
                          result: toolCall.result
                        },
                        ...providerMetadata ? { providerMetadata } : {}
        ...
        options: {
          ...
          onStepFinish: async (props) => {
            ...
            if (!isNaN(remainingTokens) && remainingTokens > 0 && remainingTokens < 2e3) {
              ...
              await chunkGCRPNAAR_cjs.delay(10 * 1e3);
            }
          },
...
async function prepareMemoryStep({
  ...
  if (typeof messages === "string") {
    ...
    const messageList = new chunkYZMSJKAK_cjs.MessageList({
      threadId: thread?.id,
      resourceId: thread?.resourceId
    });
  ...
  __primitive({
    ...
    return {
      before: async () => {
        ...
        let messageList = new chunkYZMSJKAK_cjs.MessageList({
          threadId,
          resourceId,
      ...
      after: async ({
        ...
        const messageListResponses = new chunkYZMSJKAK_cjs.MessageList({
          threadId,
          resourceId,
          generateMessageId: this.capabilities.mastra?.generateId?.bind(this.capabilities.mastra),
          // @ts-expect-error Flag for agent network messages
...
var prepareMemoryStepOutputSchema = z10.z.object({
  ...
  messageList: z10.z.instanceof(chunkYZMSJKAK_cjs.MessageList),
...
var Agent = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async #validateRequestContext(requestContext) {
    if (this.#requestContextSchema && chunkGCRPNAAR_cjs.isZodType(this.#requestContextSchema)) {
      const contextValues = requestContext?.all ?? {};
      const validatedRequestContext = await this.#requestContextSchema.safeParseAsync(contextValues);
      if (!validatedRequestContext.success) {
...
${errorMessages}`,
  ...
  listTools({ requestContext = new chunk5WBEMKE2_cjs.RequestContext() } = {}) {
    if (typeof this.#tools !== "function") {
      return chunkGCRPNAAR_cjs.ensureToolProperties(this.#tools);
    }
    ...
    return resolveMaybePromise(result, (tools) => {
      ...
      return chunkGCRPNAAR_cjs.ensureToolProperties(tools);
    });
  }
  /**
  ...
  getLLM({
    ...
    return resolveMaybePromise(modelToUse, (resolvedModel) => {
      ...
      if (isSupportedLanguageModel(resolvedModel)) {
        ...
        llm = new chunkOOTLMVNN_cjs.MastraLLMV1({
          model: resolvedModel,
          mastra: this.#mastra,
          options: { tracingPolicy: this.#options?.tracingPolicy }
        });
  ...
  async resolveModelConfig(modelConfig, requestContext) {
    try {
      return await chunkPP4G2TZZ_cjs.resolveModelConfig(modelConfig, requestContext, this.#mastra);
    } catch (error) {
      const mastraError = new chunk4U7ZLI36_cjs.MastraError({
  ...
  async generateTitleFromUserMessage({
    ...
    const normMessage = new chunkYZMSJKAK_cjs.MessageList().add(message, "user").get.all.aiV5.ui().at(-1);
    if (!normMessage) {
    ...
    if (isSupportedLanguageModel(llm.getModel())) {
      const messageList = new chunkYZMSJKAK_cjs.MessageList().add(
        [
          {
            role: "system",
  ...
  async genTitle(userMessage, requestContext, observabilityContext, model, instructions) {
    try {
      if (userMessage) {
        const normMessage = new chunkYZMSJKAK_cjs.MessageList().add(userMessage, "user").get.all.ui().at(-1);
        if (normMessage) {
  ...
  async listMemoryTools({
    ...
    if (memoryTools) {
      ...
      for (const [toolName, tool2] of Object.entries(memoryTools)) {
        ...
        const convertedToCoreTool = chunkGCRPNAAR_cjs.makeCoreTool(toolObj, options, void 0, autoResumeSuspendedTools);
  ...
  async listWorkspaceTools({
    ...
    const workspaceTools = chunkMEMYFFOL_cjs.createWorkspaceTools(workspace);
  ...
  async listSkillTools({
    ...
    const skillTools = chunkMEMYFFOL_cjs.createSkillTools(workspace.skills);
  ...
  async listAssignedTools({
    ...
    const assignedCoreToolEntries = await Promise.all(
      assignedToolEntries.map(async ([k, tool2]) => {
        ...
        return [k, chunkGCRPNAAR_cjs.makeCoreTool(tool2, options, void 0, autoResumeSuspendedTools)];
      })
    );
    ...
    if (toolsFromToolsets.length > 0) {
      ...
      for (const toolset of toolsFromToolsets) {
        for (const [toolName, tool2] of Object.entries(toolset)) {
          ...
          const convertedToCoreTool = chunkGCRPNAAR_cjs.makeCoreTool(toolObj, options, "toolset", autoResumeSuspendedTools);
  ...
  async listClientTools({
    ...
    if (clientToolsForInput.length > 0) {
      ...
      for (const [toolName, tool2] of clientToolsForInput) {
        ...
        const convertedToCoreTool = chunkGCRPNAAR_cjs.makeCoreTool(toolRest, options, "client-tool", autoResumeSuspendedTools);
        toolsForRequest[toolName] = convertedToCoreTool;
      }
    }
    return toolsForRequest;
  ...
  async listAgentTools({
    ...
    if (Object.keys(agents).length > 0) {
      for (const [agentName, agent] of Object.entries(agents)) {
        ...
        const toolObj = chunkM26GEN4C_cjs.createTool({
          id: `agent-${agentName}`,
...
${effectiveInstructions}`;
        ...
        convertedAgentTools[`agent-${agentName}`] = chunkGCRPNAAR_cjs.makeCoreTool(
          toolObj,
          options,
          void 0,
          autoResumeSuspendedTools
  ...
  async listWorkflowTools({
    ...
    if (Object.keys(workflows).length > 0) {
      for (const [workflowName, workflow] of Object.entries(workflows)) {
        ...
        convertedWorkflowTools[`workflow-${workflowName}`] = chunkGCRPNAAR_cjs.makeCoreTool(
          toolObj,
          options,
          void 0,
          autoResumeSuspendedTools
  ...
  async convertTools({
    ...
    if (this.#mastra) {
      mastraProxy = chunkGCRPNAAR_cjs.createMastraProxy({ mastra: this.#mastra, logger });
    }
  ...
  async prepareModels(requestContext, model) {
    if (model || !Array.isArray(this.model)) {
      ...
      let headers;
      if (resolvedModel instanceof chunkPP4G2TZZ_cjs.ModelRouterLanguageModel) {
    ...
    const models = await Promise.all(
      this.model.map(async (modelConfig) => {
        ...
        let headers;
        if (model2 instanceof chunkPP4G2TZZ_cjs.ModelRouterLanguageModel) {
  ...
  async #execute({ methodType, resumeContext, ...options }) {
    ...
    if ("structuredOutput" in options && options.structuredOutput?.schema) {
      ...
      if (targetProvider.includes("openai") || targetModelId.includes("openai")) {
        if (chunkGCRPNAAR_cjs.isZodType(options.structuredOutput.schema) && targetModelId) {
          const modelInfo = {
  ...
  async generate(messages, options) {
    ...
    const mergedOptions = chunkGCRPNAAR_cjs.deepMerge(
      defaultOptions,
      options ?? {}
    );
  ...
  async resumeStream(resumeData, streamOptions) {
    ...
    let mergedStreamOptions = chunkGCRPNAAR_cjs.deepMerge(
      defaultOptions,
      streamOptions ?? {}
    );
    const llm = await this.getLLM({
...
var ToolSearchProcessor = class {
  ...
  constructor(options) {
    ...
    this.bm25Index = new chunkMEMYFFOL_cjs.BM25Index({}, TOOL_SEARCH_TOKENIZE_OPTIONS);
    this.indexTools();
    if (this.ttl > 0) {
  ...
  async processInputStep(args) {
    ...
    const searchTool = chunkM26GEN4C_cjs.createTool({
      id: "search_tools",
    ...
    const loadTool = chunkM26GEN4C_cjs.createTool({
      id: "load_tool",
...
var SemanticRecall = class {
  ...
  formatCrossThreadMessages(messages, currentThreadId) {
    let result = ``;
    const v1Messages = new chunkYZMSJKAK_cjs.MessageList().add(messages, "memory").get.all.v1();
    let lastYmd = null;
...
https://mastra.ai/en/docs/memory/semantic-recall`
      ...
      if (typeof config.embedder === "string") {
        this.embedder = new chunkPP4G2TZZ_cjs.ModelRouterEmbeddingModel(config.embedder);
      } else {
...
https://mastra.ai/en/docs/memory/overview`
  ...
  setEmbedder(embedder, embedderOptions) {
    if (typeof embedder === "string") {
      this.embedder = new chunkPP4G2TZZ_cjs.ModelRouterEmbeddingModel(embedder);
    } else {
  ...
  getMergedThreadConfig(config) {
    ...
    const mergedConfig = chunkGCRPNAAR_cjs.deepMerge(this.threadConfig, config || {});
...
var MockMemory = class extends MastraMemory {
  constructor({
    ...
    super({
      name: "mock",
      storage: storage || new chunk7AHCLTZZ_cjs.InMemoryStore(),
      options: {
  ...
  listTools(_config) {
    ...
    return {
      updateWorkingMemory: chunkM26GEN4C_cjs.createTool({
...
//# sourceMappingURL=chunk-BCSVBOAN.cjs.map
//# sourceMappingURL=chunk-BCSVBOAN.cjs.map