...
var StepExecutor = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkBCSVBOAN_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await step.execute(
        chunkBCSVBOAN_cjs.createDeprecationProxy(
          {
            ...
            getStepResult: chunkBCSVBOAN_cjs.getStepResult.bind(this, stepResults),
            suspend: async (suspendPayload, suspendOptions) => {
              const { suspendData, validationError: validationError2 } = await chunkBCSVBOAN_cjs.validateStepSuspendData({
          ...
          {
            paramName: "runCount",
            deprecationMessage: chunkBCSVBOAN_cjs.runCountDeprecationMessage,
            logger: this.logger
          }
        )
      );
      ...
      return {
        ...
        tripwire: error instanceof chunkBCSVBOAN_cjs.TripWire ? {
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkBCSVBOAN_cjs.TripWire(
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunk7AHCLTZZ_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkBCSVBOAN_cjs.createTimeTravelExecutionParams({
    ...
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkBCSVBOAN_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkBCSVBOAN_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkBCSVBOAN_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkM26GEN4C_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkBCSVBOAN_cjs.Agent) && !(input instanceof chunkM26GEN4C_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkBCSVBOAN_cjs.Agent) && !(obj instanceof chunkM26GEN4C_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const isV2Model = chunkBCSVBOAN_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: chunkBCSVBOAN_cjs.ProcessorStepSchema,
    outputSchema: chunkBCSVBOAN_cjs.ProcessorStepOutputSchema,
    execute: async ({ inputData, requestContext, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkBCSVBOAN_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          ...
          if (error instanceof chunkBCSVBOAN_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkYZMSJKAK_cjs.MessageList) {
                ...
                chunkBCSVBOAN_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkBCSVBOAN_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
...
var EventedWorkflow = class extends chunkBCSVBOAN_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkBCSVBOAN_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkBCSVBOAN_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
...
var EventedExecutionEngine = class extends chunkBCSVBOAN_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkBCSVBOAN_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkBCSVBOAN_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-Z2X5VTYJ.cjs.map
//# sourceMappingURL=chunk-Z2X5VTYJ.cjs.map