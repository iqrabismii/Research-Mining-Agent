...
var chunkXJNQHPBJ_cjs = require('../chunk-XJNQHPBJ.cjs');
...
var planCounter = 0;
var askUserTool = chunkM26GEN4C_cjs.createTool({
  id: "ask_user",
...
var submitPlanTool = chunkM26GEN4C_cjs.createTool({
  id: "submit_plan",
...
var taskWriteTool = chunkM26GEN4C_cjs.createTool({
  id: "task_write",
...
var taskCheckTool = chunkM26GEN4C_cjs.createTool({
  id: "task_check",
...
function createSubagentTool(opts) {
  ...
  return chunkM26GEN4C_cjs.createTool({
    id: "subagent",
...
- The task is self-contained and can be delegated`,
    ...
    execute: async ({ agentType, task, modelId }, context) => {
      ...
      const subagent = new chunkBCSVBOAN_cjs.Agent({
...
var Harness = class {
  ...
  constructor(config) {
    ...
    if (config.workspace instanceof chunkMEMYFFOL_cjs.Workspace) {
  ...
  async init() {
    if (this.config.storage) {
      this.#internalMastra = new chunkXJNQHPBJ_cjs.Mastra({ logger: false, storage: this.config.storage });
      await this.#internalMastra.getStorage().init();
    }
    if (this.config.workspace && !this.workspaceInitialized && !this.workspaceFn) {
      try {
        if (!this.workspace) {
          this.workspace = new chunkMEMYFFOL_cjs.Workspace(this.config.workspace);
        }
  ...
  async getCurrentModelAuthStatus() {
    ...
    try {
      const { PROVIDER_REGISTRY } = await import('../provider-registry-EHOAWHFE.cjs');
