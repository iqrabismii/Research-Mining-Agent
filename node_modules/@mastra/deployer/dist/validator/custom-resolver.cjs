...
async function getParentPath(specifier, url) {
  ...
  const matchedPackage = Object.keys(importers).find((external) => chunkG7R456H2_cjs.isDependencyPartOfPackage(specifier, external));
  if (!matchedPackage) {
    return null;
  }
...
async function resolve(specifier, context, nextResolve) {
  ...
  if (stubbedExternals.length > 0) {
    const isStubbed = stubbedExternals.some((ext) => chunkG7R456H2_cjs.isDependencyPartOfPackage(specifier, ext));
    if (isStubbed) {
