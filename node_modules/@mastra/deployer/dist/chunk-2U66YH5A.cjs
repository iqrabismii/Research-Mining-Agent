...
var chunkUPVHAZ5Q_cjs = require('./chunk-UPVHAZ5Q.cjs');
var chunk73SEUI3W_cjs = require('./chunk-73SEUI3W.cjs');
var chunkR2SYO6PH_cjs = require('./chunk-R2SYO6PH.cjs');
...
var Bundler = class extends bundler.MastraBundler {
  ...
  createBundler(inputOptions, outputOptions) {
    return chunkR2SYO6PH_cjs.createBundler(inputOptions, outputOptions);
  }
  async getUserBundlerOptions(mastraEntryFile, outputDirectory) {
    ...
    try {
      const bundlerOptions = await chunkUPVHAZ5Q_cjs.getBundlerOptions(mastraEntryFile, outputDirectory);
  ...
  async analyze(entry, mastraFile, outputDirectory) {
    return await chunk73SEUI3W_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap, enableEsmShim, externals }) {
    const { workspaceRoot } = await chunk73SEUI3W_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    const inputOptions = await chunkR2SYO6PH_cjs.getInputOptions(
      mastraEntryFile,
      analyzedBundleInfo,
      this.platform,
      {
  ...
  getAllToolPaths(mastraDir, toolsPaths = []) {
    const normalizedMastraDir = chunkG7R456H2_cjs.slash(mastraDir);
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    try {
      ...
      analyzedBundleInfo = await chunk73SEUI3W_cjs.analyzeBundle(
    ...
    for (const [dep, depInfo] of analyzedBundleInfo.externalDependencies) {
      ...
      try {
        let rootPath = await chunkYIUDHDPS_cjs.getPackageRootPath(dep, projectRoot);
        if (!rootPath) {
          rootPath = await chunkYIUDHDPS_cjs.getPackageRootPath(dep, undefined);
        }
        if (rootPath) {
...
//# sourceMappingURL=chunk-2U66YH5A.cjs.map
//# sourceMappingURL=chunk-2U66YH5A.cjs.map