...
async function getPackageJSON(pkgName, importer) {
  const pkgRoot = await chunkYIUDHDPS_cjs.getPackageRootPath(pkgName, importer);
  if (!pkgRoot) {
...
function nodeModulesExtensionResolver() {
  ...
  return {
    ...
    async resolveId(id, importer, options) {
      if (!importer || id.startsWith(".") || id.startsWith("/") || chunkG7R456H2_cjs.isBuiltinModule(id) || path.isAbsolute(id)) {
        return null;
      }
      ...
      const pkgName = chunkG7R456H2_cjs.getPackageName(id);
      if (!pkgName) {
        return null;
      }
      try {
        ...
        const packageRoot = await chunkYIUDHDPS_cjs.getPackageRootPath(pkgName, importer);
...
async function getInputOptions(entryFile, analyzedBundleInfo, platform, env = { "process.env.NODE_ENV": JSON.stringify("production") }, {
  ...
  const nodeResolvePlugin = nodeResolve__default.default(chunkG7R456H2_cjs.getNodeResolveOptions(platform));
  ...
  const normalizedEntryFile = chunkG7R456H2_cjs.slash(entryFile);
  return {
    ...
    plugins: [
      ...
      alias__default.default({
        entries: [
          {
            find: /^\#server$/,
            replacement: chunkG7R456H2_cjs.slash(url.fileURLToPath(undefined("@mastra/deployer/server")))
          },
          {
            find: /^\@mastra\/server\/(.*)/,
      ...
      enableEsmShim ? chunkYIUDHDPS_cjs.esmShim() : void 0,
      ...
      chunkYIUDHDPS_cjs.removeDeployer(entryFile, { sourcemap }),
...
//# sourceMappingURL=chunk-R2SYO6PH.cjs.map
//# sourceMappingURL=chunk-R2SYO6PH.cjs.map