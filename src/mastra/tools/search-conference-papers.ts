...
// Maps friendly venue names to Semantic Scholar venue query terms
...
export const searchConferencePapers = createTool({
  id: 'search-conference-papers',
  description:
    'Search papers from top AI conferences (NeurIPS, ICML, CVPR, ICLR, AAAI) and journals (Nature MI, IEEE TPAMI, TNNLS, JAIR) via Semantic Scholar.',
  ...
  execute: async ({ query, venue, maxResults, year }) => {
    ...
    const params = new URLSearchParams({
      ...
      fields:
        'paperId,title,authors,abstract,year,venue,citationCount,influentialCitationCount,url,publicationDate,isOpenAccess',
    ...
    const papers: ConferencePaper[] = (data.data ?? [])
      .filter((p) => p.venue?.toLowerCase().includes(venueName.toLowerCase().split(' ')[0]!.toLowerCase()))
      .map((p) => ({
        paperId: p.paperId,
        title: p.title,
