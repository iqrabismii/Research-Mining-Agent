...
export type ScholarPaper = z.infer<typeof scholarPaperSchema>;

export const searchSemanticScholar = createTool({
  id: 'search-semantic-scholar',
  description:
    'Search Semantic Scholar for trending and highly cited recent AI/ML papers from the past 7 days.',
  ...
  execute: async ({ query, maxResults, minYear }: { query: string; maxResults: number; minYear?: number }) => {
    ...
    const params = new URLSearchParams({
      ...
      fields:
        'paperId,title,authors,abstract,year,citationCount,influentialCitationCount,url,fieldsOfStudy,publicationDate',
    ...
    if (response.status === 429 || response.status === 503) {
      // Rate limited — return empty results so other sources still work
    ...
    const papers: ScholarPaper[] = (data.data ?? []).map((p) => ({
      paperId: p.paperId,
      title: p.title,
