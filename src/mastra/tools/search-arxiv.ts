...
export const searchArxiv = createTool({
  id: 'search-arxiv',
  description:
    'Search arxiv for recent AI/ML papers by keyword and category. Returns papers from the past 7 days.',
  inputSchema: z.object({
    query: z.string().describe('Search query, e.g. "large language models reasoning"'),
    categories: z
      .string()
      .default('cs.AI,cs.LG,cs.CL,stat.ML')
      .describe('Comma-separated arxiv categories, e.g. "cs.AI,cs.LG,cs.CL,stat.ML"'),
  ...
  execute: async ({ query, categories, maxResults }) => {
    ...
    // Parse XML manually using regex for simplicity (no xml2js needed)
